...
export default function AdminUpdateService() {
  ...
  const [data, setData] = useState({ title: "", slug: "", icon: "", gradient: GRADIENT_PRESETS[0], description: "", tagline: "", status: true });
  ...
  function getInputData(e) {
    ...
    if (["title", "slug", "icon", "gradient", "description", "tagline"].includes(name)) {
      setData(old => ({ ...old, [name]: value }));
      if (name === "title" || name === "description") setError(old => ({ ...old, [name]: formValidator(e) }));
    } else if (name === "status") {
  ...
  function postSubmit(e) {
    e.preventDefault();
    if (error.title || error.description) { setShow(true); return; }
    if (ServiceStateData.some(x => x._id !== _id && x.slug === data.slug)) {
      setShow(true); setError({ ...error, slug: "That slug is already used by another service." }); return;
    }
    ...
    // Append Image based on toggle - If left blank on Update, backend ignores and keeps old image
  ...
  useEffect(() => {
    if (loaded || !ServiceStateData.length) return;
    const item = ServiceStateData.find(x => x._id === _id);
    if (!item) return;

    setData({ title: item.title || "", slug: item.slug || "", icon: item.icon || "", gradient: item.gradient || GRADIENT_PRESETS[0], description: item.description || "", tagline: item.tagline || "", status: !!item.status });
    setOverview({ heading: item.overview?.heading || "", paragraphs: item.overview?.paragraphs?.length ? item.overview.paragraphs : [""], stats: item.overview?.stats?.length ? item.overview.stats : [emptyStat()] });
    setFeatures(item.features?.length ? item.features : [emptyFeature()]);

    setExistingImage(item.image || "");
    setLoaded(true);
  }, [ServiceStateData, _id, loaded]);
  ...
  return (
    <>
      <style>{`
        ...
        .status-pill-row { display: flex; align-items: center; gap: 8px; margin-bottom: 4px; }
        ...
        <div className="container-fluid px-3 px-lg-4 py-4">
          <div className="page-heading">
            <div className="page-heading-copy">
              ...
              <div>
                ...
                <p className="text-muted mb-0">Edit the details, overview, and features for this service.</p>
              </div>
            </div>
            <Link className="btn btn-outline-secondary btn-sm" to="/service"><i className="bi bi-arrow-left"></i> Back to Services</Link>
          </div>
          ...
          <section className="row g-3">
            <div className="col-12 col-xl-8">
              ...
              <div className="panel mb-3">
                ...
                <div className="row g-3">
                  ...
                  <div className="col-12">
                    ...
                    <div className="field-hint">Used in the service's public URL. Keep it short and unique.</div>
                  </div>
                  <div className="col-12">
              ...
              <div className="panel mb-3">
                <div className="panel-header">
                  <h2 className="h5 mb-0 section-title"><i className="bi bi-file-text"></i> Overview</h2>
                </div>
              ...
              <div className="panel mb-3">
                <div className="panel-header">
                  <h2 className="h5 mb-0 section-title"><i className="bi bi-diagram-3"></i> Features</h2>
                </div>
                {features.map((f, idx) => (
                  <div className="repeater-card" key={idx}>
                    <div className="repeater-card-head">
            ...
            <div className="col-12 col-xl-4">
              <div className="panel mb-3" style={{ position: "sticky", top: 16 }}>
                ...
                <div className="mb-3">
                  ...
                  <div className="image-drop">
                    {(imagePreview || existingImage) ? (
                      <img src={imagePreview || existingImage} alt="Service preview" />
                    ) : (
                      <i className="bi bi-image placeholder"></i>
                    )}
                  </div>

                  <label className="form-label small text-muted">Replace image</label>
                  <select className="form-select form-select-sm mb-2" value={imageSource} onChange={e => { setImageSource(e.target.value); setImagePreview(""); setImageUrl(""); }}>
                  ...
                  {imageSource === "upload" ? (
                    ...
                    <input className="form-control form-control-sm" type="url" placeholder="https://..." value={imageUrl} onChange={(e) => { setImageUrl(e.target.value); setImagePreview(e.target.value); }} />
                  )}
                  <div className="field-hint">Leave empty to keep the current image.</div>
                </div>
                ...
                <button className="btn btn-primary w-100" onClick={postSubmit}><i className="bi bi-check2-circle me-1"></i> Save Changes</button>
                <Link className="btn btn-outline-secondary w-100 mt-2" to="/service">Cancel</Link>
              </div>
            </div>
          </section>
        </div>
