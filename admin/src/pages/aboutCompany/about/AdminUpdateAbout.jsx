...
const checklist = [
  { dot: "bg-success", title: "Review Details", body: "Ensure company info and storyline are current." },
  { dot: "bg-primary", title: "Update Content", body: "Edit features, slides, values, and timeline as needed." },
      ...
      if (item) {
        setData({
          ...
          companyInfo: {
            ...
            heroSubtitle: item.companyInfo?.heroSubtitle ?? "",
            description: item.companyInfo?.description ?? "",
            mission: item.companyInfo?.mission ?? "",
            vision: item.companyInfo?.vision ?? "",
          },
          storyline: {
            eyebrow: item.storyline?.eyebrow ?? "Our Story",
            headingPrefix: item.storyline?.headingPrefix ?? "",
            headingHighlight: item.storyline?.headingHighlight ?? "",
            subheading: item.storyline?.subheading ?? "",
            body: item.storyline?.body ?? "",
            imageSrc: item.storyline?.imageSrc ?? "",
            badgeCount: item.storyline?.badgeCount ?? "200+",
            badgeLabel: item.storyline?.badgeLabel ?? "Clients served globally",
          },
          aboutFeatures: item.aboutFeatures ?? [],
          aboutSlides: item.aboutSlides ?? [],
          coreValues: item.coreValues ?? [],
          timeline: item.timeline ?? [],
        });
      }
    }
  }, [AboutStateData?.length, _id]);
        ...
        <div className="page-heading">
          <div className="page-heading-copy">
            ...
            <div>
              ...
              <h1 className="h3 mb-1">Update About Page</h1>
              <p className="text-muted mb-0">Edit the About page content below.</p>
            </div>
          </div>
            ...
            <div className="panel">
              ...
              <div className="row g-3 mb-1">
                ...
                <div className="col-md-6">
                  ...
                  {data.storyline.imageSrc && (
                    <img src={data.storyline.imageSrc} alt="Storyline preview"
                      style={{ width: 64, height: 64, objectFit: "cover", borderRadius: 8, marginTop: 8, border: "1px solid var(--admin-border)" }} />
                  )}
                </div>
              ...
              {data.aboutSlides.some((s) => s.src) && (
                <div className="d-flex flex-wrap gap-2 mt-2">
                  {data.aboutSlides.filter((s) => s.src).map((s, i) => (
                    <img key={i} src={s.src} alt={s.alt}
                      style={{ width: 64, height: 64, objectFit: "cover", borderRadius: 8, border: "1px solid var(--admin-border)" }} />
                  ))}
                </div>
              )}
              ...
              <div className="d-flex flex-wrap justify-content-end gap-2 mt-4">
                ...
                <button className="btn btn-primary" type="button" onClick={postSubmit}>
                  <i className="bi bi-check-circle" aria-hidden="true"></i> Update About Page
                </button>
              </div>
            </div>
          </div>
