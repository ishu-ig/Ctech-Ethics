...
import {
    ...
    deleteTeams,
    updateTeams,
} from "../../Redux/ActionCreators/TeamsActionCreators";
...
export default function AdminTeam() {
    ...
    function deleteRecord(_id) {
        if (window.confirm("Are you sure you want to delete this item?")) {
            dispatch(deleteTeams({ _id }));
            setFlag(!flag);
        }
    }

    function updateRecord(_id) {
        const item = TeamsStateData?.find((a) => a._id === _id);
        if (!item) return;
        dispatch(updateTeams({ _id, status: !item.status }));
        setFlag(!flag);
    }
    ...
    const totalCount = TeamsStateData ? TeamsStateData.length : 0;
    const activeCount = TeamsStateData ? TeamsStateData.filter((i) => i.status).length : 0;
    const inactiveCount = TeamsStateData ? TeamsStateData.filter((i) => !i.status).length : 0;

    const filteredData = TeamsStateData
        ? TeamsStateData.filter((item) =>
    ...
    return (
        ...
        .team-skill-badge {
          font-size: 0.7rem; margin: 0 3px 3px 0;
        }
      `}</style>
                    ...
                    <div className="page-heading">
                        <div className="page-heading-copy">
                            ...
                            <div>
                                ...
                                <h1 className="h3 mb-1">Team</h1>
                                <p className="text-muted mb-0">
                                    Review and manage team members.
                                </p>
                            </div>
                        </div>
                        <div className="heading-actions">
                            <Link className="btn btn-primary btn-sm" to="/team/create">
                                <i className="bi bi-plus-circle" aria-hidden="true"></i> Add Team Member
                            </Link>
                        </div>
                    </div>

                    {/* Metric Cards */}
                    <section className="row g-3 mt-2 mb-1" aria-label="Team summary">
                        <div className="col-12 col-sm-6 col-xl-4">
                            <article className="metric-card text-white">
                                ...
                                <div className="metric-meta"><span>all</span><span>members</span></div>
                            </article>
                        </div>
                        <div className="col-12 col-sm-6 col-xl-4">
                            <article className="metric-card text-white">
                    ...
                    <section className="panel mt-3">
                        <div className="panel-header">
                            <div>
                                ...
                                <p className="text-muted mb-0">
                                    Search, review, and manage team members.
                                </p>
                            </div>
                        ...
                        <div className="table-responsive">
                            <table className="table align-middle mb-0" id="teamTable">
                                <thead>
                                    <tr>
                                        <th scope="col">#</th>
                                    ...
                                    {filteredData.length > 0 ? (
                                        filteredData?.map((item, index) => (
                                            <tr key={item._id}>
                                                ...
                                                <td>
                                                    {item.image ? (
                                                        <img className="team-avatar" src={item.image} alt={item.name} />
                                                    ) : (
                                                        <span className="team-avatar d-inline-flex align-items-center justify-content-center bg-light">
                                                ...
                                                <td>
                                                    {item.skills && item.skills.length > 0
                                                        ? item.skills.map((s) => (
                                                            <span key={s} className="badge text-bg-light border team-skill-badge">
                                                                {s}
                                                            </span>
                                                        ))
